    ...
    <OverlayContent class='exhibitor' v-if="show" :class={bookmarked} back-mode=none @close='$store.dispatch("selectNone")'>
        <template slot="bar">
            ...
            <div class="exhibitor__bar-booth" @click='$store.dispatch("toggleMapOverlay")'>Booth
                <span v-for="booth in booths.filter(b => !b.name.startsWith('X') && !b.name.startsWith('X'))" :key="booth.id">
                    {{booth.name}}
                </span></div>
        </template>
        <div class="exhibitor__details">
            ...
            <div class="exhibitor__categories">
                <a href='' v-for="booth in booths.filter(b => !b.name.startsWith('X') && !b.name.startsWith('X'))" :key="booth.id" @click.prevent='$store.dispatch("toggleMapOverlay")' class="exhibitor__categories-booth">Booth {{booth.name}}</a>
