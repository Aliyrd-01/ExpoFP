...
const Gallery: React.FC<GalleryProps> = ({ images, leading = false }) => {
        ...
        return images.map((url, i) => {
            return (
                <div className="gallery__item" key={url + i} onClick={() => openModal(i)}>
                    <GalleryImg setHeight={leading} fillMode="cover" containerRef={rootRef} url={url} />
                </div>
            );
        });
    };
        ...
        return images.map((url, i) => {
            return (
                <SwiperSlide key={url + i}>
                    ...
                    >
                        <TransformImg swiperRef={swiperRef} rootRef={rootRef} url={url} />
                    </TransformWrapper>
                </SwiperSlide>
            );
        });
    ...
    const Controls = ({ zoomIn, zoomOut }) => (
        <div className="gallery-slider__controls">
            <button className="gallery-slider__btn close" aria-label="🗙" onClick={closeModal}>
                <svg
                    className="icon"
                    width="30"
                    height="30"
...
const TransformImg: React.FC<TransformImgProps> = ({ swiperRef, rootRef, url }) => {
    ...
    return (
        <TransformComponent wrapperClass="gallery-slider__zoom" contentClass="gallery-slider__zoom-content">
            <GalleryImg containerRef={rootRef} url={url} />
        </TransformComponent>
    );
};
