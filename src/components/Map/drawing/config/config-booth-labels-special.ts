...
class BoothLabelSpecialDrawer extends BoothDrawerBase<RectPainter> {
    ...
    constructor(context: DrawerContext, layerID: string, booth: Booth, painterOrderPriority: number, visible: boolean) {
        ...
        for (const s of this.steps) {
            const canvasTmp = createMultilineTextCanvas(
                s.lines,
                s.width,
                s.fontSize,
                booth.labelColor || settings.boothLabelColor
            );
        ...
        const dotCanvas = createCircleCanvas(1.5, context.pixelRatio, booth.labelColor || settings.boothLabelColor);
